...
int main(int argc, char *argv[]) {
    ...
    dfp("sel4utils_configure_process", error);

    printf("num of domains: %d\n", CONFIG_NUM_DOMAINS);

    seL4_CPtr domainCPtr;
    domainCPtr = simple_get_init_cap(&simple, seL4_CapDomain);

    error = seL4_DomainSet_Set(domainCPtr, 1, new_process.thread.tcb.cptr);
    dfp("seL4_DomainSet_Set", error);

    error = sel4utils_spawn_process_v(&new_process, &vka, &vspace, 0, NULL, 1);
    dfp("sel4utils_spawn_process_v", error);
    
	int i = 0;
	while (i >= 0) {
		printf("Hello World! from app1: %d\n", i++);
	}
